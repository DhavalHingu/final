...
$sql_sel_sub="select * from room_master where room_id='$room_id'";
...
<form method="post" action="../controller/roomController.php" name="form1">
<input type="hidden" name="insid" value="" /> <table border="1">

<tr><td>Room Number</td><td><input type="text" name="room_no" value="<?php echo $room_no;?>"></td></tr>
<tr>
	<td>Block Name</td>
	<td><select name="block_no">
	<option value="-">Select Block</option>
	<?php
				                       
       $sql=mysql_query("SELECT * FROM block_master");
       ...
       {
       	?>
       	<option <?php echo (($rs['block_no']==$block_no)?"selected":"") ?> value="<?php echo $rs['block_no'];?>"><?php echo $rs['block_name']; ?></option>
      <?php }
      }
...
<?php }}else {?>
 <center><h1>View Rooms </h1></center>
...
<?php
	$i=1;
 	$sql_sel_sub="select * from room_master as r,block_master as b where r.block_no=b.block_no";
 	$res_sel_sub= mysql_query($sql_sel_sub,$conn);
	...
	<tr>
		...
		<td><a href="view_rooms.php?mode=edit&room_id=<?php echo $room_id;?>"  title="Edit" ><i class="fa fa-pencil" aria-hidden="true" style="font-size: 20px;"></i></a> | <a href="../controller/roomController.php?action=delete&room_id=<?php echo $room_id;?>"  title="Delete" ><i class="fa fa-times" aria-hidden="true" style="font-size: 20px;"></i></a></td>
	</tr>
		<?php } ?>
